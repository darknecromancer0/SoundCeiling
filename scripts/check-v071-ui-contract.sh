...
require "$SIMPLE" 'globalDsp.setText("PCM Shadow (без звука)")'
...
# v0.11 replaces the Simple master with an independent desired/max card.
# Historical linked controls remain in Advanced for Shadow/Relay, disabled while linked.
...
require "$SIMPLE" 'userVolumeCard = new UserVolumeCard(context)'
require "$SIMPLE" 'userVolumeCard.refresh()'
require "$SIMPLE" 'root.addView(userVolumeCard, spaced())'
require "$PKG/UserVolumeCard.java" 'desired.setEnabled(!relay)'
require "$PKG/UserVolumeCard.java" 'maximum.setEnabled(!relay)'
require "$PKG/UserVolumeCard.java" '0.45f'
