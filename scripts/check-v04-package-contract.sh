...
require(){ local file="$1"; local needle="$2"; grep -Fq "$needle" "$file" || { echo "Missing v0.4 package regression: $file -> $needle" >&2; exit 1; }; }
# v0.4 package regressions intentionally exclude release version/artifact metadata.
# Those values advance in later releases and are enforced by the current release contract.
...
echo "v0.4 package regression: PASS"
