...
need(){ grep -Fq -- "$2" "$1" || { echo "v0.7.7.1 readback wiring contract missing: $2" >&2; exit 1; }; }
...
if grep -Fq 'evaluateEnhancedSessionNeutralAttach' "$E"; then
  echo 'v0.7.7.1 readback wiring contract: production runtime still uses acoustic attach gate' >&2
  exit 1
fi
if grep -Fq 'finishEnhancedSessionDifferentialProbe' "$E"; then
  echo 'v0.7.7.1 readback wiring contract: production runtime still uses acoustic differential gate' >&2
  exit 1
fi
