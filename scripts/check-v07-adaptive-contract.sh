...
require "$PKG/NormalizerService.java" 'safeVolume.enforceHardMax(before, safetySettings, writeAt)'
...
require "$PKG/SimpleModeView.java" 'PCM Shadow — отдельный расчёт без звука'
require "$PKG/SimpleModeView.java" 'Samsung Media подстраивается под звук автоматически.'
require "$PKG/SimpleModeView.java" 'UserVolumeCard'
...
require "$PKG/AdvancedModeView.java" 'Обычную громкость и её максимум задаёт верхняя карточка.'
require "$PKG/AdvancedModeView.java" 'Кнопки меняют только вашу громкость. При 0% — без звука и пауза; положительный уровень продолжает автогромкость.'
...
require "$PKG/AdvancedModeView.java" 'StatusText.pcmDsp(runtime)'
...
# v0.7.2 renames the same FFT visualization for clarity; the structural meter checks below remain.
...
require "$PKG/UserVolumeCard.java" 'desired = slider("Громкость SoundCeiling", false)'
require "$PKG/UserVolumeCard.java" 'maximum = slider("Максимум SoundCeiling", true)'
require "$PKG/UserVolumeCard.java" 'seek.setMax(100)'
require "$PKG/UserVolumeControl.java" 'curve.capIndexFromPercent(percent)'
...
require "$PKG/DiagnosticsView.java" 'Частотный спектр: %s · age %d ms'
