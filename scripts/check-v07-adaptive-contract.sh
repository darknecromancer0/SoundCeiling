...
require "$PKG/SimpleModeView.java" 'основная автогромкость меняет Samsung Media по одной ступени'
...
require "$PKG/AdvancedModeView.java" 'Тихое повышается, громкое снижается по одной ступени, не выше Safety Maximum'
require "$PKG/AdvancedModeView.java" 'Volume Down ставит обычное управление на паузу до Остановить → Запустить'
