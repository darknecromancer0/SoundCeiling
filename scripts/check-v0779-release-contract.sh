...
need(){ grep -Fq -- "$2" "$1" || { echo "v0.7.7.9+ historical release contract missing: $2" >&2; exit 1; }; }
...
if not m or int(m.group(1)) < 33:
    raise SystemExit('v0.7.7.9+ historical release contract: expected versionCode >= 33')
PY
...
echo 'v0.7.7.9+ historical release contract: PASS'
