...
final class EqSettings {
    ...
    static EqSettings load(Context context) {
        ...
        return new EqSettings(p.getBoolean(ENABLED, false), p.getInt(LINK_STRENGTH, 85), levels, linked);
    }
    ...
    EqSettings withEnabled(boolean value) { return new EqSettings(value, linkStrengthPercent, levelsMb, linked); }
    EqSettings withLinkStrength(int value) { return new EqSettings(enabled, value, levelsMb, linked); }
    EqSettings withLinked(int band, boolean value) {
        boolean[] next = linked.clone(); if (band >= 0 && band < next.length) next[band] = value;
        return new EqSettings(enabled, linkStrengthPercent, levelsMb, next);
    }
    EqSettings moveBand(int band, int levelMb, int minMb, int maxMb) {
        ...
        return new EqSettings(enabled, linkStrengthPercent, next, linked);
    }
