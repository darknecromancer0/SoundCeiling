...
final class HelpText {
    ...
    static String forKey(String key) {
        ...
        if (QUIET_LEVEL.equals(key)) return "Quiet Now level — верхний предел Media для кнопки Quiet Now. В интерфейсе он задаётся в процентах системной шкалы Media; внутри Android всё равно применяет ближайшую реальную ступень. Команда может только снизить текущую громкость до этого предела и никогда не повышает её.";
        if (CEILING_BASIS.equals(key)) return "Шкала управления меняет представление одних и тех же настроек Adaptive Envelope. Media % показывает системную шкалу Android/Samsung. Digital dB делает главными цифровые Target LUFS-like, Tolerance и Projected Peak dBFS; это не физическая громкость в комнате. Calibrated dB SPL использует приблизительную акустическую оценку только для текущего калиброванного выхода; без действующей калибровки остаётся Safe fallback с обычными Media-ограничениями. Ни одна шкала не создаёт права на свободное повышение: вверх допускается только ограниченное восстановление ранее доказанного собственного снижения SoundCeiling.";
